...
docker run -it ^
--mount type=bind,source=%CD%\test-project,target=c:/workspace ^
--mount type=bind,source=%CD%\regkeys,target=c:/regkeys ^
--mount type=bind,source=%CD%\output,target=c:/output ^
...
--rm --env-file ./credentials.env gameci/windows-editor