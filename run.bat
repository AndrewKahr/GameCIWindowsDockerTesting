...
docker run -it ^
--mount type=bind,source=%CD%\test-project,target=c:/workspace/test-project ^
--mount type=bind,source=%CD%\output,target=c:/workspace/output ^
...
--rm --env-file ./credentials.env --env-file ./variables.env gameci/windows-editor